...
4. 用 read 读取 `~/.config/opencode/AGENTS.md`（stats.budgets 已有数字；读正文既用于判断是否有可合并/超预算段落，也作为 Phase 3 契约冲突比对的基准——只观察，不改）。
...
- **契约/技能冲突候选**：把候选合并稿与高权威记忆的观点，同 `AGENTS.md` 及相关 skill 的**原文**逐条比对。凡出现互斥说法（规则相反、路径/字段/流程不一致、契约已下结论而记忆仍在用旧说法）→ 单列一条，附**双方原文 + memory id + `file:line`**，给出建议保留方（**默认契约/skill 优先**，除非有证据表明契约本身该改）与 `confidence`。取证只用 read/grep；绝不修改契约、技能或记忆。
...
- 预算观察：AGENTS.md 字节/行数、注入预算、技能数量（技能数已远超"配额 ≤10"设想——只记录观察，不行动）。
- 每类删候选都必须引用证据；没有证据不列。

### Phase 5 · L2/L3 进化候选（只列候选）

- **技能候选（L2）**：仅当某路径有 ≥2 次成功证据时才列入；描述做什么、验证方式、来源，并给出**建议落位**——通用技能目录（`~/.config/opencode/skills/<name>/`）还是项目目录（`<repo>/.opencode/skills/<name>/`）——以及是否建议公开发布。你只提案：不创建技能、不建目录、不建仓库、不移动已有技能；落位与发布一律等人工闸确认后才执行。
